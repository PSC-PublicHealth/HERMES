...
#        storageSC = self.getOriginalStorageSC()
    ...
    def cutGridPower(self,duration):
        ...
        if self.gridPowerCut == 1:
            ...
            if self.sim.now() < self.sim.model.burninDays:
                dur_here = self.sim.now() + duration - self.sim.model.burninDays
                if dur_here > 0.0:
                    self.timeWOPower += dur_here
            else:
...
class ShipperProcess(Process, abstractbaseclasses.UnicodeSupport):
    ...
    def run(self):
	    ...
	    if fromWHPow is False:
            	print "In Here 20"
                while fromWHPow is False:
                    yield hold, self, powerCheck 
                    fromWHPow = self.fromW.hasPower
