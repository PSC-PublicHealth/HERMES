...
import sys,os,os.path,re,types,unittest,StringIO
...
class KVPParser:
    ...
    def _innerParseKVP( self, iterator ):
        result= {}
        
        for rec in iterator:
            if self.debug: print 'parsing <%s>'%rec
            tokenizer= KVPParser._tokGen(rec)
            key= None
            val= None
            valType= None
    ...
    def writeKVP( self, ofileOrFilename, dict ):
        """
...
class TestKVPTools(unittest.TestCase):
    ...
    def test_kvptools(self):
        ...
        parser.writeKVP(outSIO,totalDict)
        inSIO = StringIO.StringIO(outSIO.getvalue())
        testDict= parser.parse(inSIO)
