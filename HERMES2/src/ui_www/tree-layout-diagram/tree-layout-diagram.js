...
;(function($) {
    ...
    $.widget("treelayout.diagram", {
		...
		change_location_text: function(levelNum, levelCount){
			...
			$("#loctext_"+levelNum).text(levelCount + " " + locString);
		},
		...
		change_route_amt: function(levelNum,flag){
			if(flag == 'true'){
				$("#link_amt_" + levelNum).text(tp[9]);
			}
			else{
				$("#link_amt_" + levelNum).text(tp[10]);
			}
		},
		
		change_route_freq: function(levelNum,flag){
			if(flag == 'true'){
				$("#link_freq_" + levelNum).text(tp[4]);
			}
			else{
				$("#link_freq_" + levelNum).text(tp[5]);
			}
		},
		...
		change_route_interv: function(levelNum,interv,ymw){
			...
			if (intervInt == intervFloat)
				$("#link_interv_" + levelNum).text(intervInt + " "+ tp[6] + " " + ymw);
			else
				$("#link_interv_" + levelNum).text(intervFloat + " " + tp[6] +" " + ymw);
		},
		...
		change_route_time: function(levelNum, timing, unit){
			...
			if (intervInt == intervFloat)
				$("#link_transit_"+levelNum).text(tp[7] +": " + intervInt + " " + unit);
			else
				$("#link_transit_"+levelNum).text(tp[7] + ": " +intervFloat + " " + unit);
		...
		change_route_distance: function(levelNum, distance){
			...
			if(intervInt == intervFloat)
				$("#link_distance_"+levelNum).text(tp[8] + ": "+ intervInt + " km");
			else
				$("#link_distance_"+levelNum).text(tp[8] + ": "+ intervFloat + " km");	
		},
    	...
    	_create: function() {
    		...
    		var thissvgContainerID = this.svgContainerID;
    		var thisoptions = this.options;
    		
    		d3.select("#"+thiscontainerID).append("svgContainer")
    			.attr("id",thissvgContainerID);
    		
    		var phrases = {0:'Products Begin Here',
    		               1:'Products Terminate Here',
    		               2:'Location',
    		               3:'Locations',
    		               4:'fixed frequency',
    		               5:'frequency as needed',
    		               6:'shipments per',
    		               7:'transit time',
    		               8:'distance',
    		               9:'fixed amount',
    		               10:'demand based amount'
    		           	};
    		console.log(thisoptions.rootPath);
    		translate_phrases(phrases,this.options.rootPath)
            .done(function(tphrases){
	           ...
	           // 
	            ...
	            var root = thisoptions.jsonData;
	
	            var startX = (width-rectW)/2;///3.75;
	            var startY = rectH;
	            var svg = d3.select("#"+thissvgContainerID).append("svg")
	            		.attr("width", width)
	            		.attr("height", height)
						...
						svg.append("text")
							...
							for(var i=0;i<linkY.length-1;i++){
								...
								.text(function(){
									...
									//STB TO DO, this will not translate yet...
									if (intervInt == intervFloat)
										return intervInt + " " + tp[6] + " " + linkBools[i][3];
									else
										return intervFloat + " " + tp[6] + " " + linkBools[i][3];
								})
								.attr("dy",".35em")
								.attr("transform",function(d){
								...
								.text(function(){
									...
									//STB TO DO, this will not translate yet...
									if (intervInt == intervFloat)
										return tp[7] + ": "+ intervInt + " " + unit;
									else
										return tp[7] + ": " +intervFloat + " " + unit;
								...
								.text(function(){
									...
									//STB TO DO, this will not translate yet...
									if (intervInt == intervFloat)
										return tp[8] + ": "+ intervInt + " km";
									else
										return tp[8] + ": " +intervFloat + " km";
