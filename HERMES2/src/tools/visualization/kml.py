...
class KMLVisualization(viz.Visualization):
    def __init__(self,outputName_,hermesRun_,hermesOutput_,kmlDate_,
                 numberOfColors_=100,columnHeight_=100,populationRadius_=0.2,
                 vaccineAvailRadius_=0.05,
                 title_=None):
        viz.Visualization.__init__(self,outputName_,hermesRun_,hermesOutput_,
                 numberOfColors_,columnHeight_,populationRadius_,
                 vaccineAvailRadius_,title_)
    ...
    def build(self):
        ...
        for storeID in self.netInfo.stores.keys():
            self.vizStores[storeID] = KMLVizStore(storeID,self.hermesOutput,self.netInfo,self)
        
        for routeID in self.netInfo.routes.keys():
            self.vizRoutes[routeID] = KMLVizRoute(routeID,self.hermesOutput,self.netInfo,self)
        
        popFolder = Folder("Population",visible_=True)
        vaccFolder = Folder("Vaccines")
        utilFolder = Folder("Utilization",opened_=True)
        ...
        for vaccine,vaccineName in self.vaccineNames:
            vaccineFolder = Folder("%s"%vaccineName)
            for storeId,store in self.vizStores.items():
...
class KMLVizStore(viz.VizStore):
    def __init__(self,storeID_,hermesOutput_,netInfo_,visualizationInfo_,sim_=None):  
        ...
        viz.VizStore.__init__(self,storeID_,hermesOutput_,netInfo_,visualizationInfo_,sim_)
    ...
    def createStorageFillTable(self,title="Storage Information"):
        ...
        storTable.addRow(['Refrigerated (2-8 C) Storage'],['b',4])
    ...
    def createPopulationListingTable(self, pDataDict,title="Population Information"):
        ...
        if pDataDict['all'] == 0:
            ...
            for peopleKey in sorted(pDataDict.keys()):
                if peopleKey != "all":
                    popTable.addRow([peopleKey,
                                     "%d"%pDataDict[peopleKey]],
                                    ["b",3,1])
        return popTable
...
### KML String Creators
    ...
    def kmlUtilizationPlacemark(self,storeType = "cooler",visibility=False):
        ...
        placemark = Placemark(name_=self.parsedName,
                                  type_=shapeType,
                                  coordinates_=(self._netInfoStore.Longitude,
                                                self._netInfoStore.Latitude),
                                  extrude_=True,
    ...
    def kmlPopulationPlacemark(self,visibility=False):
        ...
        percentOfMax = float(self.totalPeopleServedDict['all'])/float(self.visualizationInfo.maxPopulation)
...
class KMLVizRoute(viz.VizRoute):
    def __init__(self,routeID_,hermesOutput_,netInfo_,visualizationInfo_,sim_=None):
        ...
        viz.VizRoute.__init__(self,routeID_,hermesOutput_,netInfo_,visualizationInfo_,sim_)
...
def circleString(center_x, center_y, radius, height, resolution, extrude = False, pretty = False):
    ...
    for i in range(resolution, 0, -1):
        ...
        if i == resolution:
            orgPoint = (center_x + radius * math.sin(angle), center_y + radius * math.cos(angle))
        pointStringList.append("%g,%g,%g "%(center_x + radius * math.sin(angle), 
...
class Folder:
    def __init__(self,name_="Folder",opened_=False,visible_=False):
...
class htmlTable:
    ...
    def printHTML(self,majorColor,minorColor,width,border=1,pretty=False):
        htmlList = ['<table style="width:%s; border-spacing:0;border-collapse:collapse;border:%dpx;">'%(width,border)]
...
class Placemark:
    ...
    def __init__(self,name_="PlaceMark",type_="point",coordinates_=None,
...
def main():
    ...
    descBubble = DescriptionBubble("maroon","gold","100%","My Description")
