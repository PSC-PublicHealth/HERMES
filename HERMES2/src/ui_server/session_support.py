...
from dbinterface import getDBInterface
import serverconfig
...
class UISession(db_routines.Base):
...
### Rewrote this to now be recursive   
    ...
    def clearSessionData(self):
        ...
        self.models = []
        #getDBInterface().Session().commit(self)
    ...
    def getScratchDir(self):
        global _sI
        scratchDir = os.path.join(_sI.scratchDir(),self.sessionId)
    ...
    def getFromRequest(cls,bottleRequest):
        ...
        if sessionId:
            try:
                uiSession = getDBInterface().Session().query(UISession).filter_by(sessionId=sessionId).one()
...
def wrapBottleApp(bottleApp):
    plugin = UISessionPlugin()
    bottleApp.install(plugin)
    plugin = bottle_sqlalchemy.Plugin(
        getDBInterface().engine, # SQLAlchemy engine created with create_engine function.
        ...
        create_session=getDBInterface().SessionMaker
    )
    bottleApp.install(plugin)
    return bottleApp
