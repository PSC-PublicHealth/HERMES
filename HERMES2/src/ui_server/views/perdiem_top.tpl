...
<table>
<tr><td><button id="create_perdiem_from_proto_button" style="width:100%">{{!_('Create')}}</button></td></tr>
<tr><td><button id="copy_perdiem_button" style="width:100%">{{!_('Copy')}}</button></td></tr>
<tr><td><button id="paste_perdiem_button" style="width:100%">{{!_('Paste')}}</button></td></tr>
</table>
</td>
...
function boxClick(cb,event) {
	if (cb.checked) {
		...
		$.getJSON('{{rootPath}}json/type-check-type-dependencies',{name:cb.value, modelId:$("#perdiem_top_model_select").val()})
		.then( function(data) {
			if (data.success) return getImportConfirm(data, cb.value);
			else return $.Deferred().reject(data).promise();
		})
		.then( function(data) {
			return $.getJSON('{{rootPath}}json/add-type-to-model',{name:cb.value, modelId:$("#perdiem_top_model_select").val()});
		})
		.then( function(data) {
			if (data.success) return $.Deferred().resolve(data);
			else return $.Deferred().reject(data);
		})
		.done ( function(data) { $("#manage_perdiem_grid").trigger("reloadGrid"); } ) // to update checkboxes
		.fail( reportError )
		...
		dlg.html('{{_("Really remove all instances of ")}}' + cb.value 
				+ '{{_("from model ")}}' + $("#perdiem_top_model_select").val() + '{{_("?")}}');
		dlg.data('name',cb.value);
		dlg.data('modelId',$("#perdiem_top_model_select").val());
		dlg.data('cb',cb);
		dlg.dialog('open');
	};
}
...
$("#manage_perdiem_grid").jqGrid({ //set your grid id
   	url:'{{rootPath}}json/manage-perdiem-table',
    editurl:'{{rootPath}}edit/edit-perdiem.json',	
	datatype: "json",
	postData: {
		modelId: function() { return $("#perdiem_top_model_select").val(); }
	},
  ...
  rowNum:9999, // rowNum=-1 has bugs, suggested solution is absurdly large setting to show all on one page
	...
	pager: '#manage_perdiem_pager', //set your pager div id
  ...
  pginput: false, //ditto
	...
	beforeRequest: function() {
   	...
   	onSelectRow: function(id){
		if(id && id!==lastsel_perdiem){
			jQuery('#manage_perdiem_grid').jqGrid('saveRow',lastsel_perdiem);
			jQuery('#manage_perdiem_grid').jqGrid('editRow',id,{
				keys:true,
				extraparam: { modelId: function() { return $("#perdiem_top_model_select").val(); }}
			});
			lastsel_perdiem=id;
		}
	},
	gridComplete: function(){
		$(".hermes_info_button").click(function(event) {
			$.getJSON('{{rootPath}}json/perdiem-info',{name:unescape($(this).attr('id')), modelId:$("#perdiem_top_model_select").val()})
			.done(function(data) {
				if (data.success) {
...
function resize_grid() {
  var idGrid = "#manage_perdiem_grid"
	...
	$("#confirm_dialog").dialog({
		...
		buttons: {
	    	Ok: function() {
	    		...
	    		.done(function(data) {
	    			if (data.success) {
	    				$("#manage_perdiem_grid").trigger("reloadGrid"); // to update checkboxes
	    			}
	    			else {
	    				alert('{{_("Failed: ")}}'+data.msg);
		...
		buttons: {
	    	Ok: function() {
				...
				$.getJSON('{{rootPath}}check-unique-hint',
						{modelId:$("#perdiem_top_model_select").val(),typeName:newNm})
				.done(function(data) {
					if (data.success) {
						if (data.value) {
							$.getJSON('{{rootPath}}json/paste-clipboard-to-type', {
								modelId:$("#perdiem_top_model_select").val(), 
								type:"perdiems", name:newNm
							})
							.done(function(data) {
								if (data.success) {
									$("#manage_perdiem_grid").trigger("reloadGrid"); // to update checkboxes											
...
$(function() {
	...
	var btn = $("#create_perdiem_from_proto_button");
	btn.button();
	btn.click( function() {
		$("#manage_perdiem_grid").jqGrid('restoreRow',lastsel_perdiem);
		var perdiemName = null;
		var modelId = $("#perdiem_top_model_select").val();
		if (lastsel_perdiem) {
			perdiemName = $("#manage_perdiem_grid").jqGrid('getCell',lastsel_perdiem,'name');
		}
		if (!perdiemName || perdiemName==false) {
			...
			.done(function(data) {
				if (data.success) {
					if (data.value) {
						window.location = "{{rootPath}}perdiem-edit?modelId="+modelId+"&protoname='"+perdiemName+"'";			
					}
					else {
	...
	btn.click( function() {
		...
		if (!perdiemName || perdiemName==false) {
			...
			$.getJSON('{{rootPath}}json/copy-type-to-clipboard',
					{name:perdiemName, modelId:$("#perdiem_top_model_select").val()})
			.done(function(data) {
				if (data.success) {
					$("#paste_perdiem_button").button("option","disabled",false);
				}
				else {
					alert('{{_("Failed: ")}}'+data.msg);
				}
...
$(function() {
	...
	var btn = $("#paste_perdiem_button");
	btn.button({disabled:true});
	btn.click( function() {
		$.getJSON('{{rootPath}}check-may-modify?modelId='+$("#perdiem_top_model_select").val())
		.done(function(data) {
			if (data.success) {
				if (data.value) {
					$.getJSON('{{rootPath}}json/test-clipboard-type',{type:'perdiems'})
					.done(function(data) {
						if (data.success) {
							if (data.value) {
								...
								.done(function(data) {
									...
									$.getJSON('{{rootPath}}check-unique-hint',
											{modelId:$("#perdiem_top_model_select").val(),typeName:proposedName})
									.done(function(data) {
										if (data.success) {
											if (data.value) {
												$.getJSON('{{rootPath}}json/paste-clipboard-to-type', {
													modelId:$("#perdiem_top_model_select").val(), 
													type:"perdiems", name:proposedName
												})
												.done(function(data) {
													if (data.success) {
														$("#manage_perdiem_grid").trigger("reloadGrid"); // to update checkboxes											
						    					...
						    					$("#get_paste_name_dialog").data('modelId',$("#perdiem_top_model_select").val());
								...
								alert('{{_("The data on the clipboard is not a per diem Type.")}}')
							}
						}
						else {
							alert('{{_("Failed: ")}}'+data.msg);
	...
	.done(function(data) {
		if (data.success) {
			if (data.value) {
				...
				$("#paste_perdiem_button").button("option","disabled",true);
			}
		}
		else {
			alert('{{_("Failed: ")}}'+data.msg);
...
function switchModel() {
    $("#manage_perdiem_grid").jqGrid('setLabel','usedin',"{{_('Used In ')}}"+sel_model_name);
    if (sel_model_name == "HERMES Database")
	$("#manage_perdiem_grid").jqGrid('hideCol', 'usedin');
    else
	$("#manage_perdiem_grid").jqGrid('showCol', 'usedin');
    $("#manage_perdiem_grid").trigger("reloadGrid"); // to update checkboxes
    resize_grid()
}
...
$(function() {
	var sel = $("#perdiem_top_model_select");
	sel.change( function() {
		$.getJSON('{{rootPath}}json/set-selected-model', {id:$("#perdiem_top_model_select").val()})
		.done(function(data) {
