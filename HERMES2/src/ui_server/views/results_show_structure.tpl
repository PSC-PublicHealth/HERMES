...
$(function(){
		...
		dataType:'json',
		data:{name:dialogBoxName,geninfo:1,utilinfo:1,popinfo:1,storedev:1,transdev:1,vacavail:1,fillratio:1,invent:1,availplot:1},
...
var margin = 100;
treeJSON = d3.json("{{rootPath}}json/model-structure-tree-d3?modelId={{modelId}}", function(error, treeData) {
    ...
    function centerNode(source) {
        ...
        x = -source.y0;
        y = -source.x0;
        x = x * scale + parseInt(baseSvg.style('width')) / 2;
    ...
    function rightClick(d) {
		...
		if($("#"+dialogBoxName+"_dialog").length > 0){
			...
			populateStoreInfoDialog("{{rootPath}}",dialogBoxName,meta_data,"{{modelId}}",d.idcode,resId);
