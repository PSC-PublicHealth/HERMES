...
<div id="addstorexpt_slides">
	<div id="addstorexpt_slide1" class='addstorexpt_slide'>
		...
		<p class='expt_text'>
			...
			{{_('The characteristics of storage devices such as net storage capacity, temperature of storage, and costs may have a significant impact on the performance and efficiency of the supply chain.')}}
		...
		<p class='expt_text'>
			...
			{{_("Then you will be able to specify what complement of storage devices you would like to use and then HERMES will automatically create a new model based on your input.")}}
		</p>	
		<br><hr><br>
		<p class='expt_text'>
		...
		<div class='flex_cols'>
			...
			<div id='addstorexpt_opts' class='expt_txt' style='display:none'>
				...
				<label for="addstorexpt_replace" class='addstorexpt_opts_label'>
					{{_('Completely replace the existing storage at each location in the supply chain level with a new complement of storage devices.')}}
				</label>
				...
				<p>
					<label for="addstorexpt_addition" class='addstorexpt_opts_label'>
						{{_('Add to the current complement of storage devices at each location in the supply chain level.')}}
					</label>
	...
	<div id="addstorexpt_slide3" class='addstorexpt_slide'>
		<!-- This is the slide that you define the storage complement on -->
		...
		<div id="addstorexpt_explorer_complement_div" class="tri_div_right"></div>
	</div>
	</div>
...
$("#addstorexpt_explorer_all_div").typeExplorerGrid({
	...
	title: "{{_('Choose storage devices that you would like to add to the complement of devices.')}}"
});
...
addStoreBut.click(function(e){
	...
	if(selected.length == 0){
		...
		$("#addstorexpt_explorer_complement_div").typeExplorerGrid("add",selected,1);
		$("addstorexpt_slides").slideShowWithFlowControl("activateButton","next");
	}
});

$("#addstorexpt_explorer_complement_div").typeExplorerGrid({
	modelId: {{modelId}},
	typeClass:'fridges',
	checkBoxes: false,
	selectEnabled: false,
	...
	width:$("#addstorexpt_explorer_complement_div").width()-2.5,
	deletable: true,
...
function createDataObject(){
	return  {
			...
			'addDevices':$("#addstorexpt_explorer_complement_div").typeExplorerGrid("getNewTypes"),
			'deviceCounts':$("#addstorexpt_explorer_complement_div").typeExplorerGrid("getDeviceCounts"),
