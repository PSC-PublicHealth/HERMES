...
class SQLAlchemyPlugin(object):
    ...
    def apply(self, callback, route):
        ...
        if bottle.__version__.startswith('0.9'):
            route['config']['_wrapped_sqlalchemy'] = True
        else:
            route.config['_wrapped_sqlalchemy'] = True
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if commit:
                    # Check for failed JSON requests
                    if isinstance(rv, types.DictType) and 'success' in rv and not rv['success']:
                        print '############ Success==False; rolling back DB session'
                        session.rollback()
                    else:
